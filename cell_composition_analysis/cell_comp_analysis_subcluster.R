...
# ------ Get cell numbers and change in cell numbers ------
...
# Split the dataframe to E115 and E185 and
# Remove cell types in which the max cell number < 10
# Also remove cell types for which VGAM result is not available
combined_cluster_comp_E185 <- combined_cluster_comp %>%
  ...
  filter(max > 10) %>%
  filter(cluster %in% colnames(stat_test_vgam$E185)) %>%
  ungroup() %>%
  select(-max) %>%
...
combined_cluster_comp_E115 <- combined_cluster_comp %>%
  ...
  mutate(max = max(c_across(everything()))) %>%
  filter(max > 10) %>%
  filter(cluster %in% colnames(stat_test_vgam$E115)) %>%
  ungroup() %>%
  select(-max) %>%
  tibble::column_to_rownames(var = "cluster")
...
stat_test_vgam[["E115"]] <- stat_test_vgam[["E115"]][rownames(combined_cluster_comp_E115), colnames(combined_cluster_comp_E115)]
stat_test_vgam[["E185"]] <- stat_test_vgam[["E185"]][rownames(combined_cluster_comp_E185), colnames(combined_cluster_comp_E185)]

# Write all the results to tsv files
write.table(stat_test_vgam$E115, file = "VGAM_stat_results_E115.tsv", sep = "\t", col.names = NA, row.names = TRUE)
write.table(stat_test_vgam$E185, file = "VGAM_stat_results_E185.tsv", sep = "\t", col.names = NA, row.names = TRUE)
write.table(combined_cluster_comp_E115, file = "change_in_cell_numbers_mean_of_repeats_E115.tsv", sep = "\t", col.names = NA, row.names = TRUE)
write.table(combined_cluster_comp_E185, file = "change_in_cell_numbers_mean_of_repeats_E185.tsv", sep = "\t", col.names = NA, row.names = TRUE)
