...
main_cluster_comp <- read.xlsx(
  xlsxFile = cellcount_excelfile,
...
# Requires conversion of count data to dataframe
comp_df <- main_cluster_comp %>%
  tibble::rownames_to_column(var = "celltype") %>%
  pivot_longer(
    cols = where(is.numeric),
    names_to = "sample", values_to = "celltype_n"
  ) %>%
...
for (age_i in age_list) {
  ...
  for (i in seq_along(celltype_list)) {
    ...
    count_df <- cbind(
      comp_df_sub$celltype_n_norm,
      comp_df_sub$total_n_norm - comp_df_sub$celltype_n_norm
    )
...
stat_test_countdata[["E185"]] <- cbind(stat_test_Del.E185, stat_test_Dup.E185) %>%
  t()
colnames(stat_test_countdata[["E115"]]) <- rownames(main_cluster_comp)
colnames(stat_test_countdata[["E185"]]) <- rownames(main_cluster_comp)
...
# ---- Do Plotting -------
# Normalize cell comp and calculate averages bewteen repeats
saved_rownames <- rownames(main_cluster_comp)
main_cluster_comp <- main_cluster_comp %>%
...
# Fix rownames
rownames(main_cluster_comp) <- saved_rownames
...
main_cluster_comp_E185 <- main_cluster_comp %>%
  select(ends_with("E185"))
main_cluster_comp_E115 <- main_cluster_comp %>%
  select(ends_with("E115"))

# Get only the WT composition for reordering and bar plot
main_cluster_comp_E185_wt <- main_cluster_comp_E185 %>%
  select(starts_with("WT"))
main_cluster_comp_E115_wt <- main_cluster_comp_E115 %>%
  select(starts_with("WT"))

# Calculate log2FC
main_cluster_comp_E185 <- main_cluster_comp_E185 %>%
  mutate(Del.E185 = log2(Del.E185 / WT.E185)) %>%
  mutate(Dup.E185 = log2(Dup.E185 / WT.E185)) %>%
  select(-starts_with("WT"))
main_cluster_comp_E115 <- main_cluster_comp_E115 %>%
...
main_cluster_comp_E185 <- as.matrix(main_cluster_comp_E185) %>%
  t() %>%
  replace((. > 1), 1) %>%
  replace((. < -1), -1)
rownames(main_cluster_comp_E185) <- gsub(rownames(main_cluster_comp_E185),
  pattern = ".\\w*$",
  replacement = ""
)
main_cluster_comp_E115 <- as.matrix(main_cluster_comp_E115) %>%
  t() %>%
  replace((. > 1), 1) %>%
  replace((. < -1), -1)
rownames(main_cluster_comp_E115) <- gsub(rownames(main_cluster_comp_E115),
  pattern = ".\\w*$",
  replacement = ""
)

# Re-order the log2fc columns based on the decreasing order of cell composition
reorder_clusters <- TRUE
if (reorder_clusters) {
  main_cluster_comp_E115 <- main_cluster_comp_E115[, rownames(arrange(main_cluster_comp_E115_wt, across(everything(), desc)))]
  main_cluster_comp_E185 <- main_cluster_comp_E185[, rownames(arrange(main_cluster_comp_E185_wt, across(everything(), desc)))]
}

# Get the statistical test matrix also in the same column order and row_order
stat_test_vgam[["E115"]] <- stat_test_vgam[["E115"]][rownames(main_cluster_comp_E115), ]
stat_test_vgam[["E185"]] <- stat_test_vgam[["E185"]][rownames(main_cluster_comp_E185), ]
stat_test_countdata[["E115"]] <- stat_test_countdata[["E115"]][rownames(main_cluster_comp_E115), ]
stat_test_countdata[["E185"]] <- stat_test_countdata[["E185"]][rownames(main_cluster_comp_E185), ]
stat_test_vgam[["E115"]] <- stat_test_vgam[["E115"]][, colnames(main_cluster_comp_E115)]
stat_test_vgam[["E185"]] <- stat_test_vgam[["E185"]][, colnames(main_cluster_comp_E185)]
stat_test_countdata[["E115"]] <- stat_test_countdata[["E115"]][, colnames(main_cluster_comp_E115)]
stat_test_countdata[["E185"]] <- stat_test_countdata[["E185"]][, colnames(main_cluster_comp_E185)]

if (reorder_clusters) {
  pdf("Log2FC_to_WT_pie_heatmap_maincluster_ordered.pdf", width = 10, height = 10)
} else {
  pdf("Log2FC_to_WT_pie_heatmap_maincluster.pdf", width = 10, height = 10)
}

# Plot the log2fc cell composition for E115
# plot the bar graph of cell numbers in WT
main_cluster_comp_E115_wt %>%
  ggplot(aes(x = fct_reorder(row.names(.), WT.E115, .desc = TRUE),
    y = WT.E115)) +
  geom_col() +
  scale_y_log10() +
  theme(
...
main_cluster_comp_E185_wt %>%
  ggplot(aes(
    x = fct_reorder(row.names(.), WT.E185, .desc = TRUE),
    y = WT.E185
  )) +
  geom_col() +
  scale_y_log10() +
...
# Save calculated values into tables
write.table(main_cluster_comp, file = "normalized_averaged_celltype_composition.tsv", sep = "\t", col.names = NA, row.names = TRUE )
write.table(comp_df, file = "normalized_celltype_composition.tsv", sep = "\t", col.names = NA, row.names = TRUE )
